...
pub fn casperlabs_initiator(_attr: TokenStream, input: TokenStream) -> TokenStream {
    // An optional macro that loads the initating function for the contract which returns the
    // BTreeMap The macro itself does not return the BTreeMap
...
fn get_entry_points(
    ...
    // The initial empty defintion Tokenstream that will be incrementally added to with various
    // parts of the deploy function
    let mut definitions: proc_macro2::TokenStream = proc_macro2::TokenStream::new();
    // The start of the deploy function, which is mostly static and generic for any standard
    // casperlabs contract
    let mut init = quote! {
    ...
    // The two empty streams for arguments that must be passed to the deploy function and the
    // content of the contract call respectively
    ...
    // The intiating function must also be idenitfied so that the function can be placed in the
    // deploy function
    ...
    // Empty token stream that will change depending on the type of function passed to it
    // (constructor/method)
    let mut access_token: proc_macro2::TokenStream;
    // Loop over every possible function and match to see if it is indeed a function, the statment
    // could be a literal like  'use::super::*;'
    ...
    for func in funcs {
        if let syn::Item::Fn(func_body) = func {
            // If the function is not a constructor or method it will have no attributes and we must
            // ignore that function Additionally, the function could also be an
            // intiatior, which it means it should be ignored as well.
            ...
            {
                // If it is the intitator function, then set the identifier with the name of the
                // specified function Return the function definition
                let init_name = func_body.sig.ident.clone();
                let def = quote! { #func_body };
                definitions.extend(def);
                init_ident = quote! { #init_name() };
            } else if func_body.attrs.is_empty() {
                // Final path to ensure all non-associated functions are included within the
                // contract
                let def = quote! { #func_body };
                definitions.extend(def);
            }
    ...
    //  Setup the tail end of the deploy function with remains mostly generic for all casperlabs
    // contracts
    let tail = quote! {
    ...
    init.extend(tail);
    // Finally once all fragments of the deploy function, defintions for all other functions and the
    // arguments for the deploy function have been aggregated Return the indiviual pieces
...
fn get_var_declaration(
    ...
    // A helper function to parse the arguments of any function and return the associated
    // declaration for the variable, Its string representation, its identifier along with the
    // type of the arg
    let arg_name: &syn::Ident;
    let arg_type: &syn::Ident;
    match arg {
